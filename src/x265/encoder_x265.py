...
if __name__ == '__main__':
    ...
    os.system('ffmpeg -y -s %dx%d -i %s -vcodec hevc -qp %d %s'%(w+pad_w,h+pad_h,'image.yuv',quality,'image_comp.265'))
    os.system('ffmpeg -y -i %s -vcodec rawvideo -s %dx%d %s'%('image_comp.265',w+pad_w,h+pad_h,'image_comp.yuv'))
    ...
    img_Y = img[0][0]
    img_Y_test=np.asarray(img_Y,np.float32).reshape([1, height, width,1])
    ...
    img_compressed_Y_test=np.asarray(img_compressed_Y,np.float32).reshape([1, height, width,1])
    ...
    #restore ckpt
    saver=tf.train.import_meta_graph('/home/zanz/AR-CNN-master/encoder/model_x265/ckpt_quality=2/ckpt_04/model.ckpt.meta')#Maybe you need to change the directory
    saver.restore(sess,tf.train.latest_checkpoint("/home/zanz/AR-CNN-master/encoder/model_x265/ckpt_quality=2/ckpt_04/"))
    ...
    reconstruct_Y = reconstruct_Y.round().astype(np.float32).reshape([1,height,width,1])
    loss =sess.run(tf.reduce_mean(tf.square(reconstruct_Y - img_Y_test)))
    ori_loss = sess.run(tf.reduce_mean(tf.square(img_compressed_Y_test - img_Y_test)))
    PSNR_rec = 10.0 * math.log(65025.0 / loss) / math.log(10.0)
    PSNR_x265 = 10.0 * math.log(65025.0 / ori_loss) / math.log(10.0)
    print ('Loss: %f, Original_loss: %f, Loss decrease: %.3f%%' % (loss,ori_loss,((ori_loss - loss)/ ori_loss * 100)))
    print ('PSNR_rec: %fdB, PSNR_x265: %fdB, PSNR enhancement: %.3fdB' % (PSNR_rec,PSNR_x265,(PSNR_rec - PSNR_x265)))
