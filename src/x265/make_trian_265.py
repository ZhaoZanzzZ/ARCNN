...
def makepaths():
    data_dir = '../ProcessedData_x265/train';
    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
    
    pad_path = '../ProcessedData_x265/pad'
    if not os.path.exists(pad_path):
        os.makedirs(pad_path)

    yuv_path = '../ProcessedData_x265/yuv'
    if not os.path.exists(yuv_path):
        os.makedirs(yuv_path)
        
    x265_path = '../ProcessedData_x265/x265'
    if not os.path.exists(x265_path):
        os.makedirs(x265_path)
        
    compyuv_path = '../ProcessedData_x265/compyuv'
...
def process_train_data(setname, sub_img_size, stride, quality):
    ...
    for i in range(len(fileNames)):
        ...
        h = image.size[1]
        pad_h = 4 - h % 4
        os.system('ffmpeg -y -i %s -vf pad=%d:%d:0:0 %s'%(srcset_path + image_path,w+pad_w,h+pad_h,'../ProcessedData_x265/pad/'+'pad'+fileNames[i]))
        os.system('ffmpeg -y -i %s -s %dx%d -pix_fmt yuv420p %s'%('../ProcessedData_x265/pad/'+'pad'+fileNames[i],w+pad_w,h+pad_h,'../ProcessedData_x265/yuv/'+'src'+fileNames[i]+'.yuv'))
        os.system('ffmpeg -y -s %dx%d -i %s -vcodec hevc  -q:v %d %s'%(w+pad_w,h+pad_h,'../ProcessedData_x265/yuv/'+'src'+fileNames[i]+'.yuv',quality,'../ProcessedData_x265/x265/'+'comp'+fileNames[i]+'.265'))
        os.system('ffmpeg -y -i %s -vcodec rawvideo -s %dx%d %s'%('../ProcessedData_x265/x265/'+'comp'+fileNames[i]+'.265',w+pad_w,h+pad_h,'../ProcessedData_x265/compyuv/'+'comp'+fileNames[i]+'.yuv'))
        
        width=w+pad_w
        height=h+pad_h
        img=yuv_import('../ProcessedData_x265/yuv/'+'src'+fileNames[i]+'.yuv',(height,width),1,0) 
        img_Y = img[0][0]
        img_compressed=yuv_import('../ProcessedData_x265/compyuv/'+'comp'+fileNames[i]+'.yuv',(height,width),1,0) 
