...
class Train(object):
    def __init__(self, data_dir,batch_size, quality):
    ...
    def next_batch(self):
        pixels = HEIGHT * WIDTH * CHANNEL#32*32*1=>1024
        ...
        for i in range(batch_size):#batch_size=128
            ...
            truth = self.data[(self.current_index * 2 * pixels):((self.current_index * 2 + 1) * pixels)].reshape([WIDTH, HEIGHT,CHANNEL])
            compres = self.data[((self.current_index * 2 + 1) * pixels):((self.current_index * 2 + 2) * pixels)].reshape([WIDTH, HEIGHT,CHANNEL])


            truth_batch[i, :, :, :] = truth.astype(np.float32) / 255.0 * 100
            compres_batch[i, :, :, :] = compres.astype(np.float32) / 255.0 * 100
...
class BSDS500(object):
    def __init__(self, data_path,batch_size,quality,):
        self.train = Train(data_path,batch_size, quality)
        self.test = Test(data_path,batch_size, quality)