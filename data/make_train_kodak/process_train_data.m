function process_train_data(setname, sub_img_size, stride, quality)%eg('train', 32, 10, 10)，设置步长的原因是提高数据集的多样性
    data_dir = fullfile('..', 'ProcessedData_kodak', 'train');%fullfile构成地址字符串
    ...
    file_list = dir(fullfile('..', 'kodak',  setname, '*.png'));
    for i = 1:length(file_list)
        img = imread(fullfile('..', 'kodak',setname, file_list(i).name));%imread()函数返回的是 Mat,一个矩阵
